...
- **Tiered reveal + hidden-modality matrix (shipped on `main`):** [SPE-781](https://linear.app/spectranoir/issue/SPE-781) reveal slices (PR #2342–#2347); matrix slices [SPE-2281](https://linear.app/spectranoir/issue/SPE-2281)–[SPE-2286](https://linear.app/spectranoir/issue/SPE-2286) (PR #2403–#2415) — domain compose, weekly orchestration, modality report copy, persistent recon cache, false-entity / structural-illusion lifecycle (`src/domain/hiddenStateIllusionLifecycle.ts`), mode-specific tells (`src/domain/hiddenStateModalityTells.ts`).
- **Remaining under [SPE-70](https://linear.app/spectranoir/issue/SPE-70) umbrella:** optional post-matrix modality families (signature masking, false-detection output, glamour, etc. per `architecture/hidden-state-displacement-counter-detection.md`); optional infiltration content depth — queue detail in `planning/backlog.md`.

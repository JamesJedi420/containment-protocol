...
- **Tiered reveal + hidden-modality matrix (shipped on `main`):** [SPE-781](https://linear.app/spectranoir/issue/SPE-781) reveal slices (PR #2342–#2347); matrix slices [SPE-2281](https://linear.app/spectranoir/issue/SPE-2281)–[SPE-2285](https://linear.app/spectranoir/issue/SPE-2285) (PR #2403–#2411) — domain compose, weekly orchestration, modality report copy, persistent recon cache, false-entity / structural-illusion lifecycle (`src/domain/hiddenStateIllusionLifecycle.ts`).
- **Remaining under [SPE-70](https://linear.app/spectranoir/issue/SPE-70) umbrella:** mode-specific tells and observer-threshold validation; optional post-matrix modality families (signature masking, false-detection output, glamour, etc. per `architecture/hidden-state-displacement-counter-detection.md`); optional infiltration content depth — queue detail in `planning/backlog.md`.

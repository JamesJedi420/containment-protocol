...
- Optional `GameState.caseScopedPrerequisiteProcessingOrders` (SPE-2757) is keyed by workshop work-order ID and carries a real open case owner, recipe/material declaration, and direct prerequisite IDs. Hydration uses a plain-record, unsafe-key-rejecting sanitizer; malformed, closed-case, duplicate, or key/ID-mismatched siblings drop independently. This envelope creates no reservation, queue work, completion output, or lifecycle mutation.

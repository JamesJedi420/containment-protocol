...
- Optional `GameState.equipmentInstances` (SPE-2828 / SPE-2842) is keyed by immutable prototype-safe instance ID and stores a known equipment definition, authoritative stored/equipped location, operational/damaged condition, and an optional safe resource payload with exact integer `0 <= remaining <= capacity` bounds. Aggregate `inventory` remains authority for uninstantiated quantities; instance creation decrements it once, while relocation and instance-backed unequip/replacement/transfer never credit or duplicate stock. The explicit ordinary-instance destruction command accepts only safe, stored, payload-free, non-Combat-Stim, recovery-unclaimed identities, deletes only the selected registry key, and emits `equipment.instance_destroyed` with the exact ID, canonical definition ID/name, condition, and `manual_disposal` reason. It does not credit aggregate or material stock, mutate damaged aggregate or fabricated/recovery authorities, or persist a tombstone; stale repeats are no-ops without duplicate events. Hydration defaults legacy saves to `{}`, validates key/ID agreement, definitions, agents, allowed slots, condition, and payload, processes IDs deterministically, stores later conflicting equipped claims, and lets the first valid instance claim override the definition-only slot projection. Valid destruction events hydrate as history without recreating absent identities; malformed siblings are dropped without invention. The optional registry and destruction event do not change `GAME_STORE_VERSION`, `GAME_SAVE_VERSION`, or the operation-event schema version.

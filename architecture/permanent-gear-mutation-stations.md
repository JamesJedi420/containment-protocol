...
Two authored stations are live: `blast_door_integrity_bench` and `pressure_seal_integrity_bench`. `applyBlastDoorIntegrityLabor` mutates a stored `blast_door` identity in place; `applyPressureSealIntegrityLabor` mutates a stored `pressure_seal` identity (`stationMutation` + `cycleCount` +1) without replacing the instance ID. Catalog re-aggregation and fabricated ordinary return-to-lot fail closed when that stamp is present so rematerialize cannot orphan history onto a new UUID. Hydration keeps a stamp only when it matches the instance class; mixed pairing, interlock, and unknown station ids drop. Full SPE-113 tags, operators, black-market legality, curse catalog, and an interlock bench remain design-only. See `planning/spe-877-mutation-stations-integrity-labor-slice.md` and `planning/spe-877-pressure-seal-integrity-bench-slice.md`.

...
- **SPE-99 Slice B:** when `fieldBase` is present on an in-progress case, mission reward `reasons` and weekly report `teamStatus` include the resolved recovery mode (`ExpeditionRecoveryMode` in `models.ts`) and a legibility line describing deployed fatigue scaling (`formatExpeditionRecoveryLegibilityFromMode` in `expeditionRecoveryNode.ts`; player-facing strings in `data/expeditionRecoveryCopy.ts` and `data/fieldBaseStagingCopy.ts`). Weekly `teamStatus` uses week-open `recoveryLookup` (same snapshot as deployed fatigue) so legibility survives case resolution; open contract previews use staging copy (`would … if committed`). In-progress deployments without a valid `fieldBase` packet surface an explicit ordinary-rest / missing-staging line. Operations Front Desk team cards show `recoverySummary` and a recovery-mode tag. Shared builder: `sim/reportTeamStatus.ts`.

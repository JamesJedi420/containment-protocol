...
- **SPE-99 Slice B:** when `fieldBase` is present on an in-progress case, mission reward `reasons` and weekly report `teamStatus` include the resolved recovery mode (`unsafe_pause` through `sanctuary_recovery`) and a legibility line describing deployed fatigue scaling (`formatExpeditionRecoveryLegibilityLine` in `expeditionRecoveryNode.ts`).

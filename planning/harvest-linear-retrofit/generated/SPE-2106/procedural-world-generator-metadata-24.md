**Harvest retrofit (rich)** — `procedural-world-generator-metadata-24` → **SPE-2106** (part 1/1)
...
- **Candidates on SPE-2106:** C5, C6, C8, C10, C14, C15
---

#### C5 — Planetary/latent biological transformation-state sites
...
- **Harvest summary:** Planetary/latent biological transformation-state sites
...
- **Table note:** Planetary/latent biological transformation-state sites
...
**Traceability:** `planning/procedural-world-generator-metadata-24-harvest.md` (C5)

---

#### C6 — Conditional-access geography 
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Conditional-access geography (alignment/ritual/state gates)
- **Pattern context:** Abstracted from batch source (Partial Scribd/metadata access to procedural world/city generator supplements (pattern-only). No imported setting names, landmark titles, pantheon labels, or source prose.).
- **Repo anchor:** `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Table note:** Conditional-access geography (alignment/ritual/state gates)
...
**Traceability:** `planning/procedural-world-generator-metadata-24-harvest.md` (C6)

---

#### C8 — Persistent dimensional-scar contamination zones
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Persistent dimensional-scar contamination zones
- **Pattern context:** Abstracted from batch source (Partial Scribd/metadata access to procedural world/city generator supplements (pattern-only). No imported setting names, landmark titles, pantheon labels, or source prose.).
- **Repo anchor:** `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Table note:** Persistent dimensional-scar contamination zones

**4. Ownership & reconciliation**
- **Primary (this comment):** SPE-2106
- **Co-owners:** SPE-88, SPE-58
...
**Traceability:** `planning/procedural-world-generator-metadata-24-harvest.md` (C8)
...
#### C14 — Landmark emotional/behavioral environment modifiers
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Landmark emotional/behavioral environment modifiers
- **Pattern context:** Abstracted from batch source (Partial Scribd/metadata access to procedural world/city generator supplements (pattern-only). No imported setting names, landmark titles, pantheon labels, or source prose.).
- **Repo anchor:** `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Table note:** Landmark emotional/behavioral environment modifiers
...
**Traceability:** `planning/procedural-world-generator-metadata-24-harvest.md` (C14)

---

#### C15 — Adaptive social-hub architecture under anomaly conditions
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Adaptive social-hub architecture under anomaly conditions
- **Pattern context:** Abstracted from batch source (Partial Scribd/metadata access to procedural world/city generator supplements (pattern-only). No imported setting names, landmark titles, pantheon labels, or source prose.).
- **Repo anchor:** `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `progressClocks.ts`; `districtSchedule.ts` + `caseGeneration.ts` (SPE-109); `regionPackets.ts`; `mapMetadata.ts` access tiers; `relationshipProjection.ts`; faction runtime in `models.ts`.
- **Table note:** Adaptive social-hub architecture under anomaly conditions
...
**Traceability:** `planning/procedural-world-generator-metadata-24-harvest.md` (C15)

**Harvest retrofit (rich)** — `layered-case-concept-generator-metadata-70` → **SPE-371** (part 1/1)
...
- **Candidates on SPE-371:** C17, C24, C30, C60
---

#### C17 — Investigation format selector
...
- **Harvest summary:** Investigation format selector
...
- **Table note:** Investigation format selector
...
**Traceability:** `planning/layered-case-concept-generator-metadata-70-harvest.md` (C17)

---

#### C24 — Nonlinear reconstruction format
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Nonlinear reconstruction format
- **Pattern context:** Abstracted from batch source (Readable 12-page case concept generator PDF (layered investigation premise tables). Pattern-only — no setting names, myth labels, Rifts/Mythos/Logos terminology, or source prose.).
- **Repo anchor:** `caseGeneration.ts`; `contracts.ts` / case templates; `caseTemplates.*`; `missionIntakeRouting.ts`; `branchContinuity.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `caseGeneration.ts`; `contracts.ts` / case templates; `caseTemplates.*`; `missionIntakeRouting.ts`; `branchContinuity.ts`.
- **Table note:** Nonlinear reconstruction format
...
**Traceability:** `planning/layered-case-concept-generator-metadata-70-harvest.md` (C24)
...
#### C60 — Format-specific clue topology
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Format-specific clue topology
- **Pattern context:** Abstracted from batch source (Readable 12-page case concept generator PDF (layered investigation premise tables). Pattern-only — no setting names, myth labels, Rifts/Mythos/Logos terminology, or source prose.).
- **Repo anchor:** `caseGeneration.ts`; `contracts.ts` / case templates; `caseTemplates.*`; `missionIntakeRouting.ts`; `branchContinuity.ts`.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `caseGeneration.ts`; `contracts.ts` / case templates; `caseTemplates.*`; `missionIntakeRouting.ts`; `branchContinuity.ts`.
- **Table note:** Format-specific clue topology
...
**Traceability:** `planning/layered-case-concept-generator-metadata-70-harvest.md` (C60)

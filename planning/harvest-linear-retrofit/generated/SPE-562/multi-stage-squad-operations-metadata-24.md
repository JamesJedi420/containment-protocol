**Harvest retrofit (rich)** — `multi-stage-squad-operations-metadata-24` → **SPE-562** (part 1/1)
...
- **Candidates on SPE-562:** C6, C20, C23
---

#### C6 — Time-sensitive escalation while player elsewhere
...
- **Harvest summary:** Time-sensitive escalation while player elsewhere
...
- **Table note:** Time-sensitive escalation while player elsewhere
...
**Traceability:** `planning/multi-stage-squad-operations-metadata-24-harvest.md` (C6)

---

#### C20 — Deferred consequence propagation
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Deferred consequence propagation
- **Pattern context:** Abstracted from batch source (Partial wiki/guide metadata (sci-fi squad RPG trilogy patterns). Pattern-only — no franchise names, character labels, morality axis names, mission titles, or wiki URLs.).
- **Repo anchor:** `teamComposition.ts` (coverage roles, weakest-link); `deploymentReadiness.ts`; `relationshipProjection.ts` / chemistry; `branchContinuity.ts` (SPE-1760); `progressClocks.ts`; `legitimacy`/faction reputation; `responderDutyEvaluation.ts` (escort); `aggregateBattle.ts` parallel objective phase (abstract resolution pattern).
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `teamComposition.ts` (coverage roles, weakest-link); `deploymentReadiness.ts`; `relationshipProjection.ts` / chemistry; `branchContinuity.ts` (SPE-1760); `progressClocks.ts`; `legitimacy`/faction reputation; `responderDutyEvaluation.ts` (escort); `aggregateBattle.ts` parallel objective phase (abstract resolution pattern).
- **Table note:** Deferred consequence propagation
...
**Traceability:** `planning/multi-stage-squad-operations-metadata-24-harvest.md` (C20)

---

#### C23 — Crisis timelines do not pause indefinitely
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Crisis timelines do not pause indefinitely
- **Pattern context:** Abstracted from batch source (Partial wiki/guide metadata (sci-fi squad RPG trilogy patterns). Pattern-only — no franchise names, character labels, morality axis names, mission titles, or wiki URLs.).
- **Repo anchor:** `teamComposition.ts` (coverage roles, weakest-link); `deploymentReadiness.ts`; `relationshipProjection.ts` / chemistry; `branchContinuity.ts` (SPE-1760); `progressClocks.ts`; `legitimacy`/faction reputation; `responderDutyEvaluation.ts` (escort); `aggregateBattle.ts` parallel objective phase (abstract resolution pattern).
...
**3. Repo / subsystem anchor**
- `teamComposition.ts` (coverage roles, weakest-link); `deploymentReadiness.ts`; `relationshipProjection.ts` / chemistry; `branchContinuity.ts` (SPE-1760); `progressClocks.ts`; `legitimacy`/faction reputation; `responderDutyEvaluation.ts` (escort); `aggregateBattle.ts` parallel objective phase (abstract resolution pattern).
- **Table note:** Crisis timelines do not pause indefinitely
...
**Traceability:** `planning/multi-stage-squad-operations-metadata-24-harvest.md` (C23)

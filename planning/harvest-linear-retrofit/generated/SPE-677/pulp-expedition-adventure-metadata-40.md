**Harvest retrofit (rich)** — `pulp-expedition-adventure-metadata-40` → **SPE-677** (part 1/1)
...
- **Candidates on SPE-677:** C13, C26, C33
---

#### C13 — Rival expedition race + contested objective
...
- **Harvest summary:** Rival expedition race + contested objective
...
- **Table note:** Rival expedition race + contested objective
...
**Traceability:** `planning/pulp-expedition-adventure-metadata-40-harvest.md` (C13)

---

#### C26 — Rival progress clock with confidence limits
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Rival progress clock with confidence limits
- **Pattern context:** Abstracted from batch source (Partial Scribd index/snippets plus official product page metadata (**not** full 85/130-page extraction). Pulp expedition RPG patterns — **no** imported game title, engine name, adventure names, careers/boons/flaws text, South Seas tropes, or scenario prose.).
- **Repo anchor:** `teamComposition.ts` / `TEAM_COVERAGE_ROLES`; `missionIntakeRouting.ts`; `stealthLeaveBehindRegistry.ts` (custody); `architecture/maritime-strategy-staged-naval-action.md`; SPE-160 episodic pacing.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `teamComposition.ts` / `TEAM_COVERAGE_ROLES`; `missionIntakeRouting.ts`; `stealthLeaveBehindRegistry.ts` (custody); `architecture/maritime-strategy-staged-naval-action.md`; SPE-160 episodic pacing.
- **Table note:** Rival progress clock with confidence limits
...
**Traceability:** `planning/pulp-expedition-adventure-metadata-40-harvest.md` (C26)

---

#### C33 — Rivals use permits, records, bribery — not combat only
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Rivals use permits, records, bribery — not combat only
- **Pattern context:** Abstracted from batch source (Partial Scribd index/snippets plus official product page metadata (**not** full 85/130-page extraction). Pulp expedition RPG patterns — **no** imported game title, engine name, adventure names, careers/boons/flaws text, South Seas tropes, or scenario prose.).
- **Repo anchor:** `teamComposition.ts` / `TEAM_COVERAGE_ROLES`; `missionIntakeRouting.ts`; `stealthLeaveBehindRegistry.ts` (custody); `architecture/maritime-strategy-staged-naval-action.md`; SPE-160 episodic pacing.
...
**3. Repo / subsystem anchor**
- `teamComposition.ts` / `TEAM_COVERAGE_ROLES`; `missionIntakeRouting.ts`; `stealthLeaveBehindRegistry.ts` (custody); `architecture/maritime-strategy-staged-naval-action.md`; SPE-160 episodic pacing.
- **Table note:** Rivals use permits, records, bribery — not combat only
...
**5. Boundary**
**In scope (when owner ships):**
- Authoring guardrails and acceptance notes on SPE-677
...
**Traceability:** `planning/pulp-expedition-adventure-metadata-40-harvest.md` (C33)

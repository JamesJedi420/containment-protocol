**Harvest retrofit (rich)** — `osr-emergent-fieldplay-metadata-60` → **SPE-854** (part 1/1)
...
- **Candidates on SPE-854:** C3, C30, C48, C57
---

#### C3 — Rumor tables + confidence states
...
- **Harvest summary:** Rumor tables + confidence states
...
- **Table note:** Rumor tables + confidence states
...
**Traceability:** `planning/osr-emergent-fieldplay-metadata-60-harvest.md` (C3)

---

#### C30 — Extraction load risk-reward
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Extraction load risk-reward
- **Pattern context:** Abstracted from batch source (Readable design-article pass (emergent sandbox / rules-lite fieldplay principles). Pattern-only — no imported RPG names, article prose, fantasy encounter examples, or proprietary OSR terminology.).
- **Repo anchor:** `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
- **Table note:** Extraction load risk-reward
...
**Traceability:** `planning/osr-emergent-fieldplay-metadata-60-harvest.md` (C30)

---

#### C48 — Mission failure as region state change
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Mission failure as region state change
- **Pattern context:** Abstracted from batch source (Readable design-article pass (emergent sandbox / rules-lite fieldplay principles). Pattern-only — no imported RPG names, article prose, fantasy encounter examples, or proprietary OSR terminology.).
- **Repo anchor:** `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
- **Table note:** Mission failure as region state change
...
**Traceability:** `planning/osr-emergent-fieldplay-metadata-60-harvest.md` (C48)

---

#### C57 — No hidden mandatory content
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** No hidden mandatory content
- **Pattern context:** Abstracted from batch source (Readable design-article pass (emergent sandbox / rules-lite fieldplay principles). Pattern-only — no imported RPG names, article prose, fantasy encounter examples, or proprietary OSR terminology.).
- **Repo anchor:** `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
...
**3. Repo / subsystem anchor**
- `mapMetadata.ts`; `districtSchedule.ts` (rare events); `shared/outcomes.ts`; `equipment.ts`; `authorityGraph` rumor confidence; `recruitment` rumorTags; `siteGeneration` multi-entry patterns.
- **Table note:** No hidden mandatory content
...
**Traceability:** `planning/osr-emergent-fieldplay-metadata-60-harvest.md` (C57)

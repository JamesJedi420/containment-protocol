**Harvest retrofit (rich)** — `post-release-tactical-manual-delta-100` → **SPE-98** (part 1/1)
...
- **Candidates on SPE-98:** C9, C10–C11, C56–C66
---

#### C9 — Internal logistics transfer ≠ external barter
...
- **Harvest summary:** Internal logistics transfer ≠ external barter
...
- **Table note:** Internal logistics transfer ≠ external barter
...
**Traceability:** `planning/post-release-tactical-manual-delta-100-harvest.md` (C9)

---

#### C10–C11 — Carry burden + leadership capacity
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** Carry burden + leadership capacity
- **Pattern context:** Abstracted from batch source (Visible PDF (158 pp). **Delta pass** — emphasizes companion doctrine, follower autonomy, reputation propagation, map discovery gradients, active skill targeting, hidden dialogue checks, trade insults, disposition presets, and guardrails. Supplements `post-release-tactical-manual-metadata-104` and `sealed-facility-manual-metadata-95`; do not duplicate children.).
- **Repo anchor:** `src/domain/siteGeneration/mapMetadata.ts`; `src/domain/stealthLeaveBehindRegistry.ts`; SPE-49/SPE-98 Done substrates.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `src/domain/siteGeneration/mapMetadata.ts`; `src/domain/stealthLeaveBehindRegistry.ts`; SPE-49/SPE-98 Done substrates.
- **Table note:** Carry burden + leadership capacity
...
**Traceability:** `planning/post-release-tactical-manual-delta-100-harvest.md` (C10–C11)

---

#### C56–C66 — UI/inventory friction deltas
...
**2. Mechanic (agent-readable)**
- **Harvest summary:** UI/inventory friction deltas
- **Pattern context:** Abstracted from batch source (Visible PDF (158 pp). **Delta pass** — emphasizes companion doctrine, follower autonomy, reputation propagation, map discovery gradients, active skill targeting, hidden dialogue checks, trade insults, disposition presets, and guardrails. Supplements `post-release-tactical-manual-metadata-104` and `sealed-facility-manual-metadata-95`; do not duplicate children.).
- **Repo anchor:** `src/domain/siteGeneration/mapMetadata.ts`; `src/domain/stealthLeaveBehindRegistry.ts`; SPE-49/SPE-98 Done substrates.
- **Runtime behavior (when owner ships):** Add or extend pure simulation/authoring rules so the pattern is testable in the owning module(s) — persist state in canonical game state, surface only where the owner already plans UI/reporting.
...
**3. Repo / subsystem anchor**
- `src/domain/siteGeneration/mapMetadata.ts`; `src/domain/stealthLeaveBehindRegistry.ts`; SPE-49/SPE-98 Done substrates.
- **Table note:** UI/inventory friction deltas
...
**Traceability:** `planning/post-release-tactical-manual-delta-100-harvest.md` (C56–C66)

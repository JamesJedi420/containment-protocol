...
| **Status** | Shipped — SPE-2286 / PR #2415                                                                    |
...
## Prerequisite (on `main` @ `64225023`; shipped on `main` @ `6fe92526`)
...
- [x] Concealed fixture with tell tag: weekly report includes concealment tell readout; `hiddenState` stays non-`revealed`.
- [x] Displaced fixture with route-timing tell: readout references decoy/route inconsistency without full reveal.
- [x] Disguised fixture with speech/metadata tell: readout fires; disguise validation path unchanged.
- [x] Observer-threshold-strict: tell suppressed when capability ≥ concealment, fires when below.
- [x] `npm run lint` + targeted `npm run test:run` green; slices 1–5 regression tests unchanged.

# SPE-2992 — Project legal onto ethics-review staging

| Field               | Value                                                                                                                                                 |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Status**          | **Recently shipped**                                                                                                                                  |
| **Linear**          | [SPE-2992](https://linear.app/spectranoir/issue/SPE-2992/project-legal-onto-ethics-review-staging)                                                    |
...
| **Predecessor**     | [SPE-2990](https://linear.app/spectranoir/issue/SPE-2990/project-staging-closet-onto-procurement-logistics-staging) — already **Done**; do not reopen |
...
| **Branch**          | `cursor/spe-2992-legal-ethics-review-layout-staging-34c7`                                                                                             |
| **Base `main` SHA** | `2f0dfa81a17c48c04bbc8b9ec9700cd203cd2ee9`                                                                                                            |

## Goal

Add one authored facility-layout room beside the SPE-2987 archive pair, the SPE-2988 med_bay pair, the SPE-2989 armory pair, and the SPE-2990 staging_closet pair. Fail closed. Do not build a catalog scanner.
...
| Current behavior  | SPE-2987, SPE-2988, SPE-2989, and SPE-2990 project `archive` → `department:records-analysis`, `med_bay` → `department:emergency-response`, `armory` → `department:field-containment`, and `staging_closet` → `department:procurement-logistics`. `legal` parses and stays unprojected.   |
| Expected behavior | The same function also writes `department:ethics-review` adjacent/adjacent when `legal` is adjacent to critical. Archive, med_bay, armory, and staging_closet behavior stay. Omit layout still returns the saved map by reference.                                                       |
...
| Docs              | This slice doc; backlog handoff + manifest; SPE-2990 deferred owner pointer. No `SCHEMA_REGISTRY` change.                                                                                                                                                                                |

## Boundary

### In scope

- Fifth explicit pair in `projectFacilityLayoutRoomsOntoDepartmentLocalStaging`
- Authored rule: room `legal` with `adjacentToCritical: true` writes `department:ethics-review` to `{ inputStaging: 'adjacent', outputStaging: 'adjacent' }`
- The department id is the known registry constant `department:ethics-review`. It is not computed from the room string. `legal` and `ethics-review` share no tokens
...
- A false `legal` flag does not clear a saved ethics-review entry and does not clear archive → records-analysis, med_bay → emergency-response, armory → field-containment, or staging_closet → procurement-logistics
...
- Targeted tests: all five authored rooms together (week-close grants 2 work units on each; an unmapped sibling stays at 1), legal false and unmapped `command` stay baseline, hydration and week-close do not apply the projection

### Out of scope

- Remaining `FACILITY_ROOM_IDS` entries other than `archive`, `med_bay`, `armory`, `staging_closet`, and `legal`: `evidence_intake`, `containment_cell`, `command`, `staff_housing`, `lounge`, `recovery`, `memorial`, `briefing`, `director_office`, `finance`, `ethics_review`, `internal_affairs`
...
- Reopening SPE-2990, SPE-2989, SPE-2988, SPE-2987, SPE-2986, SPE-1026, SPE-2889, SPE-1027, or SPE-2984
...
- [x] All five authored rooms project together; week-close grants 2 work units on records-analysis, emergency-response, field-containment, procurement-logistics, and ethics-review; an unmapped sibling stays at 1
- [x] `legal` with `adjacentToCritical: false` does not insert ethics-review and does not write `remote`; a saved ethics-review entry stays; archive, med_bay, armory, and staging_closet `true` in the same layout still write their departments
...
- [x] Hydration does not apply the projection. Week-close with a snapshot that includes adjacent `legal` and omitted staging does not invent a staging map
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Remaining room-to-department catalog     | [SPE-2996](https://linear.app/spectranoir/issue/SPE-2996/project-finance-onto-concept-embodiment-research-staging) | SPE-2996 authors `finance` → `department:concept-embodiment-research`; remaining rooms stay deferred on that child. Do not map `ethics_review` → `department:ethics-review` or `evidence_intake` → `department:general-intake`. |

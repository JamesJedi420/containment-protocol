# SPE-3004 — Spread one zone-spanning event by panic

| Field               | Value                                                                                                                                                   |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Status**          | **Recently shipped**                                                                                                                                    |
| **Linear**          | [SPE-3004](https://linear.app/spectranoir/issue/SPE-3004/spread-one-zone-spanning-event-by-panic)                                                       |
...
| **Prerequisite**    | [SPE-3003](https://linear.app/spectranoir/issue/SPE-3003/spread-one-zone-spanning-event-by-visibility) — existing record and visibility rule only       |
| **Branch**          | `cursor/spe-3004-zone-spanning-panic-4283`                                                                                                              |
| **Base `main` SHA** | `f8bb5044d6fb4a8a588101dd42336996f74116ed`                                                                                                              |

## Goal

One additional rule id on the existing zone-spanning record. Panic resolves only from pairs the facility graph already has as `spatial_adjacency` edges. A missing panic source fails closed and does not write affected ids.
...
| Current behavior  | SPE-3003 applies `visibility` beside `airflow` and `spatial_adjacency`. The production graph has no panic field. SPE-2932 rejects a non-adjacency edge class.                     |
| Expected behavior | `applyZoneSpanningPanic` writes affected ids, origin removed, only along existing edges named by an optional `panic` list. A missing source returns the same record.              |
| Boundary          | One rule id, contract tests, this slice doc, and backlog handoff. No new edge class. No production topology edit. No visibility or airflow behavior change.                       |
| Risks             | Treating a missing source as an adjacency, airflow, or visibility write. Inventing an edge for a pair the graph does not already have.                                            |
| Validation        | `src/test/zoneSpanningSiteEvent.contract.test.ts`, lint, `npm run test:run`, `npm run verify:backlog-handoff`                                                                     |
| Docs              | This slice doc; `planning/spe-3003-zone-spanning-visibility-slice.md` deferred row; `planning/backlog.md`; `planning/backlog-handoff-manifest.json`. No `SCHEMA_REGISTRY` change. |

## Boundary

### In scope

- Rule id `panic` beside `visibility`, `airflow`, and `spatial_adjacency`
- `applyZoneSpanningPanic` reads an optional `panic` list of `{ fromNodeId, toNodeId }` on an authored topology
...
- A missing source (production read, omitted list, malformed list, or no pair that is already an edge) returns the input record, including when a visibility or airflow list is present
- `propagateSiteEventOverFacilityTopology` validates topology and origin. Its adjacency result is not copied, and the airflow and visibility results are not copied, when the panic lookup fails

### Out of scope

- SPE-3003 visibility behavior, SPE-3002 airflow behavior, and SPE-3001 pulse cadence
- SPE-2932 topology contents and SPE-2994 walk behavior
- Alarm and contamination spread
...
`applyZoneSpanningPanic` is pure. It does not author edges. The production graph has no panic source, so a production read returns the same record. Hydration and week-close do not call this function.

## Acceptance

- [x] An authored panic list of existing edges leaves the origin off the affected list
- [x] A missing panic source keeps the affected array reference, including when a visibility or airflow list is present
- [x] A pair that is not an existing spatial edge does not add that node
- [x] Visibility, airflow, adjacency, and the SPE-3001 pulse stay unchanged
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- |
| Alarm spread                             | [SPE-3005](https://linear.app/spectranoir/issue/SPE-3005/spread-one-zone-spanning-event-by-alarm)                                                                                                | One rule id on the zone-spanning record. This slice stays panic. |
| Contamination spread                     | [SPE-1606](https://linear.app/spectranoir/issue/SPE-1606/zone-spanning-event-propagation)                                                                                                        | Contamination is not this panic rule.                            |

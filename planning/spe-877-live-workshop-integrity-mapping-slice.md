...
| Mapping additional containment classes into workshop quality     | this SPE-877 child (`planning/spe-877-extra-class-workshop-quality-slice.md`) | Shipped extra-class `pressure_seal` / `interlock` mappings; field-containment blast-door path unchanged                                                                                                |
| Seed or materialize `equipment-instance-blast-door-workshop`     | this SPE-877 child (`planning/spe-877-seed-blast-door-workshop-slice.md`)     | Shipped: starting-state seeds the authored ID; mapping fail-closes missing instance (`poor`) only when omitted; `instantiateEquipmentInstance` still allocates `equipment-instance-${week}-${ordinal}` |

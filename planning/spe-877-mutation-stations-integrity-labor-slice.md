...
| **Branch**          | `cursor/spe-877-mutation-stations-6667`                                                                                                                                        |
...
Command `applyBlastDoorIntegrityLabor` applies that result through instance transition with `allowStationMutation`. Generic transitions cannot invent or rewrite the stamp. Relocate / SPE-2851 repair / SPE-2862 stabilization keep a valid stamp.

## Events and hydration

Successful labor hydrates as `equipment.instance_station_mutated` with reason `integrity_labor`. Valid events are history and do not replay the mutation. Malformed stamps drop independently. `GAME_STORE_VERSION` / `GAME_SAVE_VERSION` / operation-event schema version stay unchanged.
...
**Constraints:** Do not implement the full SPE-113 catalog. Do not spawn a new instance UUID. Do not treat mutation as a temporary buff. Do not bump `GAME_STORE_VERSION` unless hydration evidence requires it. Do not reopen SPE-2827 / SPE-2848. Do not pick SPE-2847.

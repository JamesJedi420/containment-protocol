...
| **Linear**          | [SPE-2867](https://linear.app/spectranoir/issue/SPE-2867/mutation-stations-integrity-labor) — child of [SPE-877](https://linear.app/spectranoir/issue/SPE-877/critical-equipment-integrity-and-deficiency-control) |
...
Successful labor hydrates as `equipment.instance_station_mutated` with reason `integrity_labor`. Valid events are history and do not replay the mutation. `hard_stop` events require `inService: false`; `none` and compensating continue require `inService: true`. This child dropped stamps that were not on a parsed `blast_door` identity (`malformed_station_mutation`). The later pressure-seal child keeps matching `pressure_seal` stamps and still drops mixed pairing. `GAME_STORE_VERSION` / `GAME_SAVE_VERSION` / the operation-event schema version stay unchanged.
...
| Interlock barrier zone                                         | [SPE-2868](https://linear.app/spectranoir/issue/SPE-2868/extra-class-barrier-zones-pressure-seal-interlock-membranes)                                                       | Shipped `interlock_membrane`; extra-class never writes `blast_door_membrane`                             |
...
| Seed or materialize `equipment-instance-blast-door-workshop`   | this SPE-877 child (`planning/spe-877-seed-blast-door-workshop-slice.md`)                                                                                                   | Shipped: starting-state seeds the authored SPE-2866 identity; mapping still fail-closes omitted instance |
| Additional SPE-113 stations, tags, operators, legality, curses | later SPE-877 / SPE-113 child (`planning/spe-877-pressure-seal-integrity-bench-slice.md` and `planning/spe-877-interlock-integrity-bench-slice.md` for extra-class benches) | Pressure-seal and interlock benches shipped; tags, operators, legality, and curses remain                |
| Mapping additional containment classes into workshop quality   | this SPE-877 child (`planning/spe-877-extra-class-workshop-quality-slice.md`)                                                                                               | Shipped extra-class workshop mappings + no-debit seeds; field-containment blast-door path unchanged      |

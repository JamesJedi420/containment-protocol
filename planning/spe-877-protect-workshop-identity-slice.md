...
`authored_workshop_identity`. Stabilize / inspect / relocate stay available.
...
| Mission-loss / mission-injury equipped workshop drop     | later SPE-877 child                                                                                         | `takeEquippedInstancesLostOnMissionResolution` is a different writer; relocate of seeds stays legal |
| Same-class sibling `sourceInstanceId`-gated barrier omit | later SPE-877 child (Codex P1 on PR #3628)                                                                  | SPE-2876 last-writer persist; omit on relief can drop a sibling-sourced `flow_restraint`            |

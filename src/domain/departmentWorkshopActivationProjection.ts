 ...
 * Returns departments from the registry with their completed-construction case/route
 * pairs and an `alreadyActivated` flag if a snapshot already exists for the department.
 */
export function projectDepartmentWorkshopActivationCandidates(
  ...
  const departments: DepartmentWorkshopActivationDepartmentCandidates[] = sortedDepartments.map(
    (department) => {
      ...
      const candidatesByCase: DepartmentWorkshopActivationRouteCandidate[] = []
      ...
      for (const caseId of sortedCaseIds) {
        ...
        const routes = currentCase?.mapLayer?.routes
      ...
      return { department, alreadyActivated, candidates: candidatesByCase }
    }
  )

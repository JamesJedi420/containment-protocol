...
export function inferCaseRegionTag(
	...
	if (['cyber', 'information', 'signal', 'relay', 'classified', 'infrastructure'].some((tag) => tags.has(tag))) {
		return 'network_grid'
	}
	...
	if (['cult', 'occult', 'ritual', 'spirit', 'haunting', 'possession', 'psionic'].some((tag) => tags.has(tag))) {
...
export function getResponseGridConfig(
	...
	const templateIds =
		responseGrid?.majorIncidentTemplateIds?.filter((templateId) => typeof templateId === 'string')
	...
	return {
		...
		majorIncidentTemplateIds:
			templateIds.length > 0 ? [...new Set(templateIds)] : DEFAULT_RESPONSE_GRID.majorIncidentTemplateIds,

...
export default function AffiliationPersonStatusMirrorPage() {
  ...
  const recordAffiliationFileWorkQueueReleasePackage = useGameStore(
    (state) => state.recordAffiliationFileWorkQueueReleasePackage
  )
                    ...
                    <tr key={entry.id} className="border-b border-white/5 align-top">
                      ...
                      <td className="px-2 py-2">
                        ...
                        {entry.isReleaseOutcomeRecorded ? (
                          <p className="mt-2 text-xs font-medium">
                            {entry.releaseOutcomeStatusLabel}
                          </p>
                        ) : entry.canRecordReleaseOutcome ? (
                          <button
                            type="button"
                            className="btn btn-xs btn-ghost mt-2 whitespace-nowrap"
                        ...
                        {entry.isReleasePackageRecorded ? (
                          <p className="mt-2 text-xs font-medium">
                            {entry.releasePackageStatusLabel}
                          </p>
                        ) : entry.canRecordReleasePackage ? (
                          <button
                            type="button"
                            className="btn btn-xs btn-primary mt-2 whitespace-nowrap"
                            onClick={() => recordAffiliationFileWorkQueueReleasePackage(entry.id)}
                          >
                        ...
                        {entry.isFileReleaseDeliveryRecorded ? (
                          ...
                          <button
                            ...
                            className="btn btn-xs btn-primary mt-2 whitespace-nowrap"
                            onClick={() =>
                              recordAffiliationFileWorkQueueFileReleaseDelivery(entry.id)
                            }
                          >

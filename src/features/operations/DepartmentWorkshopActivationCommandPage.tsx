...
export default function DepartmentWorkshopActivationCommandPage() {
  ...
  const selectedDepartment = view.departments.find(
    (department) => department.departmentId === departmentId
  )
  const selectedCandidate = selectedDepartment?.candidates.find(
    (candidate) => candidate.selectKey === selectKey
  )
    ...
    <section className="space-y-4" aria-label="Department workshop activation command">
      ...
      {view.isEmpty ? (
        ...
        >
              ...
              >
                ...
                {view.departments.map((department) => (
                  <option
                    ...
                    disabled={department.alreadyActivated || department.candidates.length === 0}
                  >
                    {department.departmentLabel}
                    {department.alreadyActivated ? ' (activated)' : ''}
                  </option>
                ))}
              </select>
              {selectedDepartment?.alreadyActivated ? (
                <p className="text-xs text-amber-300/80">{ui.alreadyActivatedNote}</p>
              ) : null}
            </div>

            {selectedDepartment && !selectedDepartment.alreadyActivated ? (
              <div className="space-y-1">
                <label
                ...
                >
                  ...
                  {selectedDepartment.candidates.map((candidate) => (
                    <option key={candidate.selectKey} value={candidate.selectKey}>
                      {candidate.constructionCaseId} / {candidate.routeLabel}
                    </option>
                  ))}
                </select>
              </div>
            ...
            {selectedCandidate ? (
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={slotCapacity}
                  onChange={(event) => {
                    const nextCapacity = Math.trunc(Number(event.target.value))
                    setSlotCapacity(Number.isFinite(nextCapacity) ? nextCapacity : 1)
                    setLastResult(null)
                  }}
                />
          ...
          {lastResult ? (
            <div
              className="space-y-1 rounded border border-white/10 bg-white/5 px-3 py-2"
              role="status"
              aria-live="polite"
            >
              {lastResult.state === 'activated' ? (
                ...
                <>
                  ...
                  {lastResult.reasons.map((reason) => (
                    <p key={reason.code} className="text-xs opacity-70">
                      {getDepartmentWorkshopActivationReasonLabel(reason.code)}
                    </p>
                  ))}
                </>
              )}

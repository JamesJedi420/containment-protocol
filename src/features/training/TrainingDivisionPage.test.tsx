...
describe('TrainingDivisionPage', () => {
  it('groups agents by readiness and links into agent detail', async () => {
    ...
    expect(screen.getByRole('heading', { name: /academy & coaching activity/i })).toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: /recent completions timeline/i })).toBeInTheDocument()
  ...
  it('upgrades the academy and unlocks tier-1 solo programs from the training page', async () => {
    ...
    expect(within(getAgentCard('Dr. Sato')).getByRole('button', { name: /threat assessment/i })).toBeEnabled()
  ...
  it('assigns and unassigns an instructor to a compatible trainee from the training page', async () => {
    ...
    const instructorSection = screen.getByRole('heading', { name: /instructor assignments/i }).closest('article')!
    expect(within(instructorSection).getByText(/prof\. chen/i)).toBeInTheDocument()

    await user.click(within(instructorSection).getByRole('button', { name: /assign to dr\. sato/i }))

    expect(useGameStore.getState().game.staff['ins-01']).toMatchObject({ assignedAgentId: 'a_sato' })
  ...
  it('shows trained bond summaries for teams with existing party-drill familiarity', () => {
    ...
    game.agents.a_ava.progression = {
      ...game.agents.a_ava.progression!,
      skillTree: {
        ...game.agents.a_ava.progression!.skillTree,
        trainedRelationships: { ...game.agents.a_ava.progression!.skillTree.trainedRelationships, a_rook: 2 },
      },
    }
    game.agents.a_rook.progression = {
      ...game.agents.a_rook.progression!,
      skillTree: {
        ...game.agents.a_rook.progression!.skillTree,
        trainedRelationships: { ...game.agents.a_rook.progression!.skillTree.trainedRelationships, a_ava: 2 },
      },
    }
  ...
  it('renders projected score breakdown and recommendation explanation for team drill suggestions', () => {
    useGameStore.setState({ game: createStartingState() })

    renderTrainingDivisionPage()

    const academySection = screen.getByRole('heading', { name: /academy analysis/i }).closest('article')!
    expect(within(academySection).getAllByText(/projection \(/i).length).toBeGreaterThanOrEqual(1)
    expect(within(academySection).getAllByText(/modifier deltas:/i).length).toBeGreaterThanOrEqual(1)
    expect(within(academySection).getAllByText(/why recommended:/i).length).toBeGreaterThanOrEqual(1)
  })

  it('renders pair-level relationship explainability in chemistry inspector', () => {
    const game = createStartingState()
    game.events = [
      {
    ...
    const inspector = screen.getByRole('heading', { name: /chemistry inspector/i }).closest('article')!
  ...
  it('allows triggering reconciliation for a strained pair from the panel', async () => {
    ...
    const reconciliationSection = screen
      ...
      .closest('article')!
    const pairItem = within(reconciliationSection).getByText(/ava brooks ↔ dr\. sato/i).closest('li')!
    ...
    expect(next.events.some((event) => event.type === 'agent.relationship_changed' && event.payload.reason === 'reconciliation')).toBe(true)
  ...
  it('shows completed training items in the completions timeline', () => {
    const game = createStartingState()
    game.events = [
      {
    ...
    expect(within(timeline!).getByText(/week 3: dr\. sato completed close-quarters drills/i)).toBeInTheDocument()
  })
  ...
  it('queues team drills from the team roster', async () => {
    ...
    const nightWatchCard = within(teamSection!).getByRole('link', { name: /night watch/i }).closest('li')
    ...
    expect(
      useGameStore
        .getState()
        .game.events.filter((event) => event.type === 'agent.training_started')
    ).toHaveLength(4)
  })
  ...
  it('filters roster by name search', async () => {
    ...
    const rosterSection = screen.getByRole('heading', { name: /eligible roster/i }).closest('article')!
  ...
  it('filters queue by scope', async () => {
    ...
    const rosterSection = screen.getByRole('heading', { name: /eligible roster/i }).closest('article')!
    const satoCard = within(rosterSection).getByText(/dr\. sato/i).closest('li')!

...
describe('SPE-94: Equipment Recovery Bottleneck', () => {
  it('recovers up to maintenanceSpecialistsAvailable damaged items per week, delaying the rest (player-facing)', () => {
    const state = createStartingState() as typeof createStartingState extends () => infer T
      ? T & { damagedEquipmentQueue?: string[] }
      : never
    state.damagedEquipmentQueue = ['itemA', 'itemB', 'itemC', 'itemD', 'itemE']
    if (state.agency) state.agency.maintenanceSpecialistsAvailable = 2
  ...
  it('recovers all items if capacity >= queue length (player-facing)', () => {
    const state = createStartingState() as typeof createStartingState extends () => infer T
      ? T & { damagedEquipmentQueue?: string[] }
      : never
    state.damagedEquipmentQueue = ['itemA', 'itemB']
    if (state.agency) state.agency.maintenanceSpecialistsAvailable = 5
  ...
  it('delays all items if capacity is zero (player-facing)', () => {
    const state = createStartingState() as typeof createStartingState extends () => infer T
      ? T & { damagedEquipmentQueue?: string[] }
      : never
    state.damagedEquipmentQueue = ['itemA', 'itemB', 'itemC']
    if (state.agency) state.agency.maintenanceSpecialistsAvailable = 0

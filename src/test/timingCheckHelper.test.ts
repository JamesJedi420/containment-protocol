...
describe('timingCheckHelper', () => {
  ...
  it('should reset run count on new week', () => {
    const state = createTimingCheckState();
    const week = 10;
    expect(shouldRunTimingCheck(state, 'OnSupportShortfallCheck', week)).toBe(true);
    expect(shouldRunTimingCheck(state, 'OnSupportShortfallCheck', week)).toBe(false);
    expect(shouldRunTimingCheck(state, 'OnSupportShortfallCheck', week + 1)).toBe(true);
    expect(getRunCountThisWeek(state, 'OnSupportShortfallCheck', week + 1)).toBe(1);
  });

  it('should allow custom maxPerWeek overrides', () => {
    const state = createTimingCheckState({ maxPerWeek: { OnResolutionCheck: 3 } });
    const week = 2;
    ...
    expect(getRunCountThisWeek(state, 'OnResolutionCheck', week)).toBe(3);
  });
});

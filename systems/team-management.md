...
Rotated-out operatives receive a **bounded, deterministic fatigue shave** derived from staging `medical` and `safety` bands. **Supply** band scales recoverable **material** quantities in the mission reward breakdown. Legibility lines are appended to mission reward reasons.
